                ...
                {loadingJobs ? (
                  ...
                  <div className="space-y-4">
                    {filteredJobs.map((job) => (
                      <Card 
                        key={job.id} 
                        className="hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1 border-2 border-white/50 bg-gradient-to-br from-white to-amber-50/30 overflow-hidden"
                      >
                        <CardHeader className="bg-gradient-to-r from-[#733635]/10 to-[#8b4a4a]/10 border-b border-amber-200/50">
                          <div className="flex justify-between items-start">
                            <div className="flex-1">
                              <CardTitle className="text-2xl font-bold mb-3 bg-gradient-to-r from-[#733635] to-[#8b4a4a] bg-clip-text text-transparent">
                                {job.title}
                              </CardTitle>
                              <CardDescription className="flex flex-wrap items-center gap-4 mt-2">
                                <span className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white/80 backdrop-blur-sm shadow-sm">
                                  <Building2 className="h-4 w-4 text-[#733635]" />
                                  <span className="font-semibold text-gray-700">{job.company}</span>
                                </span>
                                <span className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white/80 backdrop-blur-sm shadow-sm">
                                  <Briefcase className="h-4 w-4 text-[#733635]" />
                                  <span className="font-semibold text-gray-700">{job.location}</span>
                                </span>
                                <span className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white/80 backdrop-blur-sm shadow-sm">
                                  <Calendar className="h-4 w-4 text-[#733635]" />
                                  <span className="font-semibold text-gray-700">{new Date(job.postedDate).toLocaleDateString()}</span>
                                </span>
                              </CardDescription>
                            </div>
                            <Badge
                              className="ml-4 bg-gradient-to-r from-[#733635] to-[#8b4a4a] text-white border-0 shadow-lg px-4 py-1.5 text-sm font-semibold"
                            >
                              {job.type}
                            </Badge>
                          </div>
                        ...
                        <CardContent className="p-6">
                          ...
                          <div className="flex gap-3">
                            <Button
                              variant="outline"
                              className="border-2 border-[#733635] text-[#733635] hover:bg-[#733635] hover:text-white transition-all duration-300 font-semibold shadow-md hover:shadow-lg"
                            >
                              View Details
                            </Button>
                            {userRole === 'member' && (
                              <Button className="bg-gradient-to-r from-[#733635] to-[#8b4a4a] hover:from-[#8b4a4a] hover:to-[#733635] text-white shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105 font-semibold">
                                Apply Now
                              </Button>
                            )}
                          </div>
